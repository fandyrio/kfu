...
$sql= pg_query($dbconn, "select p.id_pasien,p.total_cost,p.id_kategori_layanan, p.id_kunjungan, b.* 
		...
		While($data= pg_fetch_assoc($sql)){
				                ...
				                if($jlh=pg_num_rows($fifo)>0){
					                ...
					                $id_details=$data['id']; /*id pasien_resep_batch*/
					                $qty=$data['qty'];
					                $harga=$data['total_cost'];
					                $id_kategori_harga = $data['id_kategori_layanan'];

					                $tv = pg_fetch_assoc(pg_query($dbconn,"select * from transaksi_invoice where id_pasien='$_POST[id_pasien]' and id_kunjungan='$_POST[id_kunjungan]'"));

					                pg_query($dbconn,"INSERT INTO transaksi_invoice_detail (id_pasien,id_invoice, id_kunjungan, jenis, harga, status_aktif, id_user, waktu_input, id_kategori_harga, id_detail, kuantitas, status_hapus, id_unit) VALUES ('$_POST[id_pasien]','$tv[id]', '$_POST[id_kunjungan]', 'O', '$harga', 'Y', '$_SESSION[login_user]', '$tgl_sekarang $jam_sekarang', '$id_kategori_harga', '$id_details', '$qty', 'N','$_SESSION[id_units]')");

					                var_dump("INSERT INTO transaksi_invoice_detail (id_pasien,id_invoice, id_kunjungan, jenis, harga, status_aktif, id_user, waktu_input, id_kategori_harga, id_detail, kuantitas, status_hapus, id_unit) VALUES ('$_POST[id_pasien]','$tv[id]', '$_POST[id_kunjungan]', 'O', '$harga', 'Y', '$_SESSION[login_user]', '$tgl_sekarang $jam_sekarang', '$id_kategori_harga', '$id_details', '$qty', 'N','$_SESSION[id_units]')");
