...
<div class="row justify-content-center">
	...
	{
		// LIST LOKET
		$.post("queue/apps/admin_init.php", function( data ){
		...
		$.ajax({
			type: "POST",
			dataType: "json",
			url: "queue/apps/last_stage.php",//request
			data: data,
	    ...
	    $('form select').change(function() {
			...
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "queue/apps/set_loket.php",//request
				data: data,
				success: function(data) {
					$(".jumbotron h1").html(data["next"]);
				}
		...
		$(".next_queue").click(function()
		{
			...
			if (loket==0) {
				...
				$.ajax({
					type: "POST",
					dataType: "json",
					url: "queue/apps/daemon.php",//request
					data: data,
					success: function(data) {
						$(".jumbotron h1").html(data["next"]);
						if (data["idle"]=="TRUE") {
		...
		function adudu(loket, counter){
			timerId = setInterval(function() {
				 $.post("queue/apps/daemon_try_cek.php", { loket : loket, counter : counter }, function(msg){
					if(msg.huft == 2){
						$(".try_queue").show();
					}
				},'JSON');
		...
		function adik_adudu(loket, counter){
			timerId_adik = setInterval(function() {
				 $.post("queue/apps/daemon_cek.php", { loket : loket, counter : counter }, function(msg){
					if(msg.huft == 2){
						$(".next_queue").show();
					}
				},'JSON');
		...
		$(".try_queue").click(function(){
			...
			if (loket==0) {
				...
				$.post("queue/apps/daemon_try.php", { loket : loket, counter : counter }, function(msg){
					if(msg.huft == 0){
						$(".try_queue").hide();
