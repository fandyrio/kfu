...
function getMinNoAntrian($idDokter, $idUnit)
{
	...
	$getDataAntrian=pg_query("SELECT 
		...
		join panggil_antrian pa on pa.id_antrian=a.id
		 where
		  a.waktu_masuk >= (now()::date)::varchar || ' 00:00:00' and a.id_unit='$idUnit' and pa.called='N' and a.status_aktif='Y' and a.status_antrian='Y' and pa.status='N' and a.id_poly::integer=(SELECT mp.id from master_karyawan mk join master_poly mp on mp.id=mk.poly_id where mk.id='$idDokter')), mp.nama, a.id_dokter from antrian a 
		join panggil_antrian pa on pa.id_antrian=a.id 
		join master_pasien mp on mp.id=a.id_pasien::integer
		where a.status_antrian='Y' and a.status_aktif='Y' and a.id_unit='$idUnit' and a.id_poly::integer=(SELECT mp.id from master_karyawan mk join master_poly mp on mp.id=mk.poly_id where mk.id='$idDokter') and pa.called='N' and pa.status='N' and a.waktu_masuk >= (now()::date)::varchar || ' 00:00:00' order by a.id asc LIMIT 1");
	...
	if($fetchDataAntrian['no_cetak_antrian']=="")
	{
		...
		from antrian a
		join panggil_antrian pa on pa.id_antrian=a.id
		 where a.waktu_masuk >= (now()::date)::varchar || ' 00:00:00' and a.id_unit='$idUnit' and pa.called='Y' and a.status_aktif='Y' and a.status_antrian='Y' and pa.status='Y' and a.id_poly::integer=(SELECT mp.id from master_karyawan mk join master_poly mp on mp.id=mk.poly_id where mk.id='63')), nama from antrian a 
		join panggil_antrian pa on pa.id_antrian=a.id 
		join master_pasien mp on mp.id=a.id_pasien::integer
		where a.status_antrian='Y' and a.status_aktif='Y' and a.id_unit='$idUnit' and a.id_poly::integer=(SELECT mp.id from master_karyawan mk join master_poly mp on mp.id=mk.poly_id where mk.id='$idDokter') and pa.called='Y' and pa.status='Y' and a.waktu_masuk >= (now()::date)::varchar || ' 00:00:00' order by a.id asc LIMIT 10");

		
		$fetchDataAntrian=pg_fetch_assoc($getDataAntrian);
		echo $fetchDataAntrian['no_cetak_antrian']."<br />";

		echo "<p style='font-size:12px;'>Nama : ".$fetchDataAntrian['nama']."</p>";
	...
	else
	{
		...
		{
			echo $fetchDataAntrian['no_cetak_antrian']."<br />";
			echo "<font size='2px'>Nama : ".$fetchDataAntrian['nama']."</font>";	
		}
		else
		{
			...
			{
				$getDataAntrian=pg_query("SELECT 
					(select min(no_cetak_antrian+1) as no_cetak_antrian 
					from antrian a
					join panggil_antrian pa on pa.id_antrian=a.id
					 where
					  a.waktu_masuk >= (now()::date)::varchar || ' 00:00:00' and a.id_unit='$idUnit' and pa.called='N' and a.status_aktif='Y' and a.status_antrian='Y' and pa.status='N' and a.id_poly::integer=(SELECT mp.id from master_karyawan mk join master_poly mp on mp.id=mk.poly_id where mk.id='$idDokter')), mp.nama, a.id_dokter from antrian a 
