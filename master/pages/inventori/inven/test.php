...
switch($_GET['act'])
{
	case "baru":
		...
		else{ $id_form = "'" .pg_escape_string($_POST['id_form']) . "'"; }
		$strength = $_POST['strength'];
		if (!isset($_POST['strength']) || empty($_POST['strength'])) {
				    $strength = 'NULL';				    
		} 
		else{ $strength = "'" .pg_escape_string($_POST['strength']) . "'"; }
		if (!isset($_POST['id_atccode']) || empty($_POST['id_atccode'])) {
				    $id_atccode = 'NULL';				    
		} 
		else{ $id_atccode = "'" .pg_escape_string($_POST['id_atccode']) . "'"; }
		 $id_kelas_mims = $_POST['id_mims'];
		if (!isset($_POST['id_mims']) || empty($_POST['id_mims'])) {
				    $id_kelas_mims = 'NULL';				    
		} 
		else{ $id_kelas_mims = "'" .pg_escape_string($_POST['id_mims']) . "'"; }

	    
	   
	   
	   // $id_opsi_billing = $_POST['id_opsi_billing'];
	    if (!isset($_POST['id_opsi_billing']) || empty($_POST['id_opsi_billing'])) {
				    $id_opsi_billing = 'NULL';				    
		} 
		else{ $id_opsi_billing = "'" .pg_escape_string($_POST['id_opsi_billing']) . "'"; }
	    ...
	    if (!isset($_POST['id_kategori']) || empty($_POST['id_kategori'])) {
			...
			$id_kategori = "'" .pg_escape_string($_POST['id_kategori']) . "'";
		 }

		if (!isset($_POST['id_pengobatan']) || empty($_POST['id_pengobatan'])) {
				    $id_pengobatan = 'NULL';				    
		} 
		else{ 
			$id_pengobatan = "'" .pg_escape_string($_POST['id_pengobatan']) . "'"; 
	    }
	    if (!isset($_POST['id_kategori_kehamilan']) || empty($_POST['id_kategori_kehamilan'])) {
				    $id_kategori_kehamilan = 'NULL';				    
		} 
		else{ 
			$id_kategori_kehamilan = "'" .pg_escape_string($_POST['id_kategori_kehamilan']) . "'"; 
	    }

	    if (!isset($_POST['id_narkotika']) || empty($_POST['id_narkotika'])) {
				    $id_narkotika = 'NULL';				    
		} 
		else{ 
			$id_narkotika = "'" .pg_escape_string($_POST['id_narkotika']) . "'"; 
	    }
	   ...
	   //$id_narkotika = $_POST['id_narkotika'];
		...
		//$cek_nama_brand = pg_query($dbconn,"SELECT id FROM inv_inventori WHERE id_brand ='$id_brand' ");
	...
	/*	$res=pg_query($dbconn,"INSERT INTO inv_inventori (
		...
		<?php
			...
			?>
			<script>
			window.alert("Obat Sudah ada di Inventori");
			</script>
			<?php
		}
	break;
