...
session_start();
            ...
            $status = $_POST['status'];
            $expected_date = DateToDatabase($_POST['expected_date']);
            $tanggal_expire = DateToDatabase($_POST['tanggal_expire']);
            $komentar = $_POST['komentar'];
            $refno = $_POST['refno'];
           
           var_dump($_POST);
            
           $res=pg_query($dbconn,"INSERT INTO po_hdr (id_supplier,id_unit, id_departemen, doc_no, doc_date, createddate, createdby, attention_to,shipping_address, delivery_address, status,expected_date, tanggal_expire, komentar, refno ) 
			VALUES(
			'".$id_supplier."',
			'$_SESSION[id_units]',
			'".$id_departemen."',		
			 ...
			 $row = pg_fetch_row($res);
				  ...
				  if($res){
  					$result=pg_query($dbconn,"INSERT INTO po_ln (id_inv,nama_brand, jumlah, id_satuan, harga_unit, total_harga,komen, id_q ,diskon_persen,diskon_amt, pajak_persen, pajak_amt, nett_total, id_hdr)
  						...
  						var_dump("INSERT INTO po_ln (id_inv,nama_brand, jumlah, id_satuan, harga_unit, total_harga,komen, id_q ,diskon_persen,diskon_amt, pajak_persen, pajak_amt, nett_total, id_hdr)
