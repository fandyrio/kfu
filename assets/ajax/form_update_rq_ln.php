<?php 
       $id=$_GET['id'];
       $result =pg_query($dbconn, " Select rq_ln.*, inv_satuan.nama as \"nama_satuan\" from rq_ln
                       INNER JOIN inv_satuan on inv_satuan.id=rq_ln.id_satuan WHERE rq_ln.id= $id");    
        $data = pg_fetch_array($result);                            
 ?>

<button class="btn btn-xs btn-danger" id="cancel_update_rq">Cancel</button>
<button class="btn btn-xs btn-success" id="simpan_form_update_rq_ln">Simpan</button>
            <form method="POST" class="form-horizontal" enctype="multipart/form-data" id="update_rquo">
