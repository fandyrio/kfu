  <?php 
  $id=$_GET['id'];
  $sql = pg_query($dbconn,"Select po_ln.*, inv_satuan.nama as \"nama_satuan\" from po_ln
                       INNER JOIN inv_satuan on inv_satuan.id=po_ln.id_satuan WHERE po_ln.id= $id");
  $data = pg_fetch_array($sql);
                          
 ?> 

<button class="btn-xs btn-danger" id="cancel_update_po">Cancel</button>
<button class="btn-xs btn-success" id="simpan_form_update_po_ln">Simpan</button>
            <form method="POST" class="form-horizontal" enctype="multipart/form-data" id="form_update_po_ln">
