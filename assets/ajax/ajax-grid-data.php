...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql = "SELECT id, code, nama ";
    $sql.=" FROM master_icd10";
    $sql.=" WHERE code ILIKE '%".$requestData['search']['value']."%' ";    // $requestData['search']['value'] contains search parameter
...
while( $row=pg_fetch_assoc($query) ) {  // preparing an array
    ...
    $nestedData[] = '<td><center>
                     ...
                     <button class="btn btn-default btn-xs btnTambahDiagnosa" id='.$row['id'].'><i class="icon-plus"></i></button>
                     </center></td>';       
