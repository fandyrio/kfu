...
error_reporting(E_ALL);
	...
	if(isset($_POST['no_antrian']))
	{
		...
		else
		{
			...
			join antrian a on a.id = pa.id_antrian
			 where a.no_cetak_antrian='$noAntrian' and a.id_unit='$idUnit' and a.status_antrian='Y' and status_aktif='Y' and a.waktu_masuk>='$today 00:00:00'";*/
				echo "sudah ditangani dokter lain";
			 //echo $id_dokter."/".$idDokter;
			}
		}
...
*/
		if($jumlahData>=1)
		{
			...
			date_default_timezone_set("Asia/Jakarta");
			$today=date("Y-m-d");
			$getDokterRoom=pg_query("SELECT * from ruang_dokter where id_karyawan='$id_dokter' and status='online' and tanggal='$today' and id_unit=$id_unit"); // id_unit belum masuk
