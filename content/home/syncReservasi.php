...
$checkLastSync=pg_query("SELECT * from last_sync_reservasi where id_unit='$id_unit'");
$jumlah=pg_num_rows($checkLastSync);

if($jumlah==0)
{
	$insert=pg_query("INSERT INTO last_sync_reservasi (tanggal, waktu,id_unit) values ('$today', '$waktu', '$id_unit')");
}
else
{
	$update=pg_query("UPDATE last_sync_reservasi set tanggal='$today', waktu='$waktu' where id_unit='$id_unit'");
}


$getLastSync=pg_query("SELECT tanggal,waktu from last_sync_reservasi where id_unit='$id_unit'");
$fetchLastSync=pg_fetch_assoc($getLastSync);
//$since=$fetchLastSync['tanggal']."T".$fetchLastSync['waktu'].".000Z";
$since="2018-04-16T04:24:00.021Z";
  			...
  			while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) 
  			{
	    		...
	    		if(isset($data[1]))
	    		{
	    			if($data[1]!='reservationId')
	    			{
	    				...
	    				{
	    					$insertNoAntrian=pg_query("INSERT into antrian_reservasi (id_reservasi,no_antrian,id_unit,tanggal,seen)
			    				VALUES ('$data[1]','$antrianReservasi','$_SESSION[id_units]','$today','N')");
			    				$antrianReservasi++;
