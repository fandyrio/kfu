	...
	<div class="animated fadeIn">
		<div class="row">
			...
			if($_GET['id_pasien']){
				...
				$getData=pg_query("select * from transaksi_invoice where id_kunjungan='$id_kunjungan'");
									$fetchData=pg_fetch_array($getData);
				//var_dump("select * from transaksi_invoice where id_kunjungan='$id_kunjungan'");
					$id_invoice=$fetchData['id'];
									//var_dump($id_invoice);
				...
				<div class="card">
							...
							<table class="table table-bordered">
								...
								<tbody>
									...
									$tampil=pg_query($dbconn,"SELECT t.* FROM transaksi_invoice_detail t
												...
												WHERE t.id_pasien='$id_pasien' AND t.id_kunjungan='$id_kunjungan' AND t.status_aktif='Y' AND t.status_hapus='N' AND i.status_issue is null ORDER BY t.id ASC");

									//var_dump("SELECT t.* FROM transaksi_invoice_detail t
										//		INNER JOIN transaksi_invoice i on i.id=t.id_invoice
										//		WHERE t.id_pasien='$id_pasien' AND t.id_kunjungan='$id_kunjungan' AND t.status_aktif='Y' AND t.status_hapus='N' AND i.status_issue is null ORDER BY t.id ASC");
									...
									while($r=pg_fetch_array($tampil))
									{
										...
										//var_dump($id_invoice);
										$c=pg_fetch_array(pg_query($dbconn,"SELECT * FROM transaksi_invoice WHERE id_pasien='$id_pasien' AND id_kunjungan='$id_kunjungan' AND id='$id_invoice' AND status_issue='1' "));
					...
					<!--  -->
						<?php
							...
							//var_dump("SELECT DISTINCT id_kategori_harga FROM transaksi_invoice_detail WHERE id_invoice='$id_invoice'");
							?>
