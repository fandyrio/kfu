...
if (empty($_SESSION['login_user'])){
	...
	if ($module=='keuangan_customer_billing' AND $act=='edit'){
		...
		<input type="hidden" name="id_kategori_harga" value="<?php echo $r[id_kategori_harga];?>">
				...
				<div class="modal-body" id="form-data">
					...
					<div class="col-sm-8"><?php										
						$result =pg_query($dbconn, "SELECT u.* FROM master_kategori_harga u 
										where u.id='4' or u.id='1' ORDER BY id");
						...
						?>
							<select name='id_kategori_harga' id='id_kategori_harga' class='form-control' required>									
										...
										while ($row =pg_fetch_assoc($result)){
											...
											echo"<option value='$r[id_kategori_harga]'>$row[nama]</option>";
											}									
									}
											?>
							</select>
					...
					<div class="form-group row">
						...
						<div class="col-md-4">
							<input type="number" name="harga"  class="form-control" value="<?php echo $r['harga']?>" required>
						</div>
					</div>
				</div>
	...
	elseif ($module=='keuangan_customer_billing' AND $act=='update'){
		...
		pg_query($dbconn,"UPDATE transaksi_invoice_detail SET kuantitas='$_POST[kuantitas]', harga='$_POST[harga]', id_user='$_SESSION[login_user]', waktu_input='$tgl_sekarang $jam_sekarang' WHERE id='$_POST[id]'");
	...
	elseif ($module=='keuangan_customer_billing' AND $act=='input'){
		...
		/**/
		pg_query($dbconn,"UPDATE transaksi_invoice SET total='$_POST[total]', diskon='$_POST[diskon]', sisa='$_POST[sisa]', status_bayar='N', status_issue='1' WHERE id='$_POST[id_invoice]'");

		$dt = pg_fetch_assoc(pg_query($dbconn,"SELECT * FROM transaksi_invoice WHERE id='$_POST[id_invoice]'"));
		$no_invoice_new = $dt[no_invoice];

		header("location:view-keuangan-customer-invoice-$no_invoice_new");
		/**/
	}
