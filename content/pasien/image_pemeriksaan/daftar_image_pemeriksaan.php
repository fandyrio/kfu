...
$a=pg_query($dbconn,"SELECT * from pasien_image_pemeriksaan where id_pasien='$d[id]'");
...
<div class="card" id="data_fisik">
	<div class="card-header">
		<strong>Image Pemeriksaan</strong>
						...
						<?php
							...
							while($r=pg_fetch_array($a)){
								...
								<tr>
									...
									<td width="30px"><button class="btn btn-xs btn-primary show" id="<?php echo $r['id'] ?>">Show</button></td>
									<td width="30px"><button class="btn btn-xs btn-danger delete" id="<?php echo $r['id'] ?>">Delete</button></td>
								</tr>
								<?php
								$no++;
							}
...
$(function () {
	$(".btnTambah").click(function(){
		...
		$.ajax({
			type: 'POST',
			url: 'form-tambah-image-pemeriksaan',
			data: dataString2,
	...
	$(".show").click(function(e)
	{
		...
		$.ajax(
		{
			url:'content/pasien/image_pemeriksaan/show_image.php',
			data:{id:id},
			type:'POST',
			success:function(result)
			{
				$("#form-modal2").html(result).modal('show'); 
			}
		});
	});
	...
	$(".delete").click(function(e)
	{
		...
		if(confirm("Press a button!")==true)
		{
			...
			$.ajax(
			{
				url:'content/pasien/image_pemeriksaan/delete_image.php',
				data:{id:id},
				type:'POST',
				success:function(result)
				{
