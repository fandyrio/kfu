			...
			<div class="card-block">
				<div class="row">
					...
					<div class="col-md-5">
						...
						<tbody>
							<?php
							$tampil=pg_query($dbconn,"SELECT * FROM transaksi_invoice_detail WHERE id_pasien='$id_pasien' AND  id_unit='$_SESSION[id_units]' AND id_kunjungan='$id_kunjungan' ORDER BY id DESC");
								...
								while($r=pg_fetch_array($tampil)){
									...
									$transaksi=pg_query($dbconn,"SELECT * FROM lab_order WHERE  id_transaksi_invoice_detail='$r[id]'  ORDER BY id ASC");
									...
									<tr>
										...
										<?php if($row['jenis']=='E'){
											...
											if($row['jenis']=='L'){
												$l=pg_fetch_array(pg_query($dbconn,"SELECT * FROM lab_analysis WHERE id='$row[id_detail]'"));
												 echo '<td colspan="3"><i>'.$l[nama].'</i></td>';
											...
											while($index_bahan=pg_fetch_array($find_bahan)){
													$nama_regensia=pg_fetch_array(pg_query($dbconn,"select  n.nama from inv_inventori i inner join inv_nama_brand n on n.id=i.id_brand
													WHERE i.id='$index_bahan[id_inv]' "));	
											 		...
											 		echo '<td></td>';
											 		echo '<td>';
													echo $nama_regensia['nama'];
													echo '</td>';
													echo '<td style="text-align:right;">';
													echo $index_bahan['jumlah'];
													echo '</td>';
													echo '</tr>';
											...
											elseif($row['jenis']=='LG'){
												$lg=pg_fetch_array(pg_query($dbconn,"SELECT * FROM lab_analysis_group WHERE id='$row[id_detail]'"));
												 echo '<td colspan="3"><i>'.$lg[nama].'</i></td></tr>';
												 $details=pg_query($dbconn,"SELECT id_lab_analysis FROM lab_analysis_group_detail WHERE id_lab_analysis_group='$lg[id]'");
													...
													$a=pg_fetch_array(pg_query($dbconn,"SELECT * FROM lab_analysis WHERE id='$index_details[id_lab_analysis]'"));


											echo '<td colspan="3">';
												echo  "<ul><li>".$a[nama]."</li></ul>";
											echo '</td>';
													$find_bahan=pg_query($dbconn,$query);
													while($index_bahan=pg_fetch_array($find_bahan)){
																$nama_regensia=pg_fetch_array(pg_query($dbconn,"select  n.nama from inv_inventori i inner join inv_nama_brand n on n.id=i.id_brand
										...
										elseif($row['jenis']=='S'){
											...
											//var_dump("SELECT * FROM lab_analysis WHERE id='$row[id_detail]'");

											echo '<td colspan="3">';
												echo "<b>ST - ". $nama_transaksi="$a[nama]"."</b>";
											echo '</td>';
							...
							<div class="tab-content">
								<div class="tab-pane active" id="tabkiri1" role="tabpanel">
									...
									<table id="example2" class="table">
											...
											<tbody>
												<?php
												$unique =array_unique ($bahan['S_Inv']);												
