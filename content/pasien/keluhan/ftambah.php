...
<div class="modal-dialog modal-md modal-success">
		...
		<input type="hidden" name="rm" value="<?php echo $_POST['rm'];?>" id="rm">
			<div class="modal-content">	
				...
				<div class="modal-body" id="form-data">
					...
					<div class="form-group" id="simptom">
						<label>Symptom </label>
						<input type="text" name="namaSymptom" id="namaSymptom" class="form-control col-md-12">
						<input type="hidden" name="jumlahSymptom" id="jumlahSympton" value="0">
					...
					<div class="form-group col-md-12" id="selectedSymptom">
	...
	<script src="assets/js/select2.full.js"></script>
		...
		{
			$("#listId").html("");
			$("#namaSymptom").keyup(function()
			{
				var namaSymptom=$("#namaSymptom").val();
				var lokasi=$("#id_lokasi").val();
				var body=$("#id_body").val();
				$.ajax(
				{
					url:'content/pasien/keluhan/daftar_symptom.php',
					data:{namaSymptom:namaSymptom, lokasi:lokasi, body:body},
					type:'GET',
					success:function(result)
					{
						$("#listSymptom").html(result).show();
					}
				});
			});
	...
	$('#btnSimpanKeluhan').click(function()
		{
			...
			var rm = $('#rm').val();
			var numOfSymptom=$(".namaSymptom").length;
			var symptomList=$("#listId").text();
			
			$.ajax({
				type: "POST",
				url: "content/pasien/keluhan/simpan.php",
				data: form+'&symptomList='+symptomList,
				cache: false,
