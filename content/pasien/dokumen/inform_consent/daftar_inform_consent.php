...
$a=pg_query($dbconn,"SELECT ic.*,mk.nama as nama_dokter FROM inform_consent_list ic
	join antrian a on a.id_kunjungan=ic.id_kunjungan
	join master_pasien mp on mp.id=a.id_pasien::integer
	join master_karyawan mk on mk.id=a.id_dokter::integer
	WHERE ic.id_pasien='$d[id]'");
						...
						<?php
							...
							while($r=pg_fetch_array($a)){
								...
								<tr>
									...
									<td><a target="_blank" href="content/pasien/dokumen/dokumen_surat/<?php echo $r['nama_file'] ?>"><button class="btn btn-xs btn-primary">Print</button></a></td>
								</tr>
								<?php
								$no++;
							}
