...
session_start();
	...
	$getDataReservasiLab=pg_query("SELECT * from data_reservasi_lab where id_kunjungan='$id_kunjungan'");
	$fetchDataReservasiLab=pg_fetch_assoc($getDataReservasiLab);
	$idLab=$fetchDataReservasiLab['id_lab'];

	?>
		<div class="col-md-12">
					<a href="content/pasien/form/createPdf.php?id_kunjungan=<?php echo $id_kunjungan ?>&id_pasien=<?php echo $id_pasien ?>" target="_blank"><button class="btn btn-primary btn-xs" style="float:right;">Print</button></a>
				</div>
		...
		<div class="card-block">
			<div class="row" style="margin-bottom:10px;">
				...
				<div class="col-md-4">
					<input type="text" name="kota_perujuk" class="form-control" id="kota_perujuk" value="<?php echo $fetchDataReservasiLab['dibuat_di'] ?>">
				</div>
					...
					<table>
						...
						while($r=pg_fetch_array($tampil)){
							?>
									...
									<td><input type="checkbox" name="checked_a[]" class="done" id='single_<?php echo $r['id'];?>_<?php echo $r['nama'] ?>'  value="<?php echo $r['id'];?>"
										<?php
										$getData=pg_query("SELECT * from reservasi_lab where id_kunjungan='$id_kunjungan'");
										while($fetchData=pg_fetch_assoc($getData))
										{
											if($r['id']==$fetchData['id_pemeriksaan'])
											{
												echo "checked";
											}
						...
						while($r=pg_fetch_array($tampil)){
							?>
									...
									<td><input type="checkbox" name="checked_a[]" class="done" id='single_<?php echo $r['id'];?>_<?php echo $r['nama'] ?>'  value="<?php echo $r['id'];?>" 
										...
										<?php
										$getData=pg_query("SELECT * from reservasi_lab where id_kunjungan='$id_kunjungan' and jenis_lab='single'");
										while($fetchData=pg_fetch_assoc($getData))
										{
											if($r['id']==$fetchData['id_pemeriksaan'])
											{
					...
					<table>					
						...
						while($r=pg_fetch_array($tampil)){
							?>
									...
									<tr>
									<td><?php echo $r[kode];?></td>									
									<td><input type="checkbox" name="checked_a[]" class="done" id='multi_<?php echo $r['id'];?>_<?php echo $r['nama'] ?>' value="<?php echo $r['id'];?>" 

										<?php
										$getData=pg_query("SELECT * from reservasi_lab where id_kunjungan='$id_kunjungan' and jenis_lab='multi'");
										while($fetchData=pg_fetch_assoc($getData))
										{
											if($r['id']==$fetchData['id_pemeriksaan'])
											{
		...
		<div class="card-footer">
			<button type="button" class="btn btn-primary btn-sm btnUpdate">Update</button>
		...
		$(document).ready(function()
		{
			...
			$(".done").click(function()
			{
				...
				{
					var indexId=arr.indexOf(id); // cek target pada arr ke berapa
	...
	/*		$("input[name='checked_a[]']").change(function() {
			...
			$(".btnUpdate").click(function()
			{
				...
				$.ajax(
				{
					url:'content/pasien/form/updateLabOrder.php',
