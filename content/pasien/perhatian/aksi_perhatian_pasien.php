...
if (empty($_SESSION['login_user'])){
	...
	elseif ($module=='perhatian' AND $act=='delete'){
		pg_query($dbconn,"UPDATE pasien_perhatian SET status_hapus='Y', status_aktif='N', waktu_input='$tgl_sekarang $jam_sekarang', id_user='$_SESSION[login_user]' WHERE id='$_POST[id]'");
	}
	
	elseif ($module=='perhatian' AND $act=='stop'){
		pg_query($dbconn,"UPDATE pasien_perhatian SET status_aktif='N' WHERE id='$_POST[id]'");
	}
	
	elseif($module=='perhatian' AND $act=='action_button'){
		$id_pasien=$_POST['id_pasien'];
		$d=pg_fetch_array(pg_query($dbconn,"SELECT no_rm FROM master_pasien WHERE id='$_POST[id_pasien]'"));
		
		$c=pg_fetch_array(pg_query($dbconn,"SELECT COUNT(id) AS tot FROM pasien_perhatian WHERE id_pasien='$id_pasien' AND status_aktif='Y' AND status_hapus='N'"));
		if($c['tot']>0){
		?>
		<button type="button" class="btn btn-warning btn-xs btnPerhatianPasien" id="<?php echo $d['no_rm'];?>"><i class="fa fa-exclamation-circle"></i></button>
		<?php
		}
